...
public class Generator
{
    ...
    public Board Generate(int d = 4, int minPly = 15, int trial = 10000)
    {
        ...
        for (int i = 0; i < trial; ++i)
        {
            ...
            while (!reversi.IsOver)
            {
                ...
                if (ply >= minPly && IsGoodForPractice(reversi.Board, d))
                {
    ...
    /// <returns></returns>
    public bool IsGoodForPractice(Board board, int d)
    {
        ...
        var candidatesDepth1 = scoresDepth1.Where(x => x.Score >= bestScoreDepth1 - d);
        ...
        if (!(scoresDepth3
            .Where(x => bestDepth3.Score - x.Score <= d)
            .Count() <= 1)
        ) return false;
